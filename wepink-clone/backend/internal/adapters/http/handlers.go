...
func (h *OrderHandler) MercadoPagoWebhook(w http.ResponseWriter, r *http.Request) {
	var reqPayload usecase.WebhookPayload
	if err := json.NewDecoder(r.Body).Decode(&reqPayload); err != nil {
		w.WriteHeader(http.StatusOK)
		return
	}

	if reqPayload.Type != "payment" {
		w.WriteHeader(http.StatusOK)
		return
	}

	if reqPayload.Data.ID == "" {
		w.WriteHeader(http.StatusOK)
		return
	}

	if err := h.paymentUseCase.HandleWebhook(r.Context(), reqPayload); err != nil {
		fmt.Printf("Webhook error processing transaction %s: %v\n", reqPayload.Data.ID, err)
	}
